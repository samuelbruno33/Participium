...
class TestBrowseReportsOnMap:
    ...
    def test_click_map_marker_shows_popup_and_opens_detail(self, driver, wait):
        ...
        wait.until(EC.presence_of_element_located((By.ID, "public-report-row-2-map-marker"))).click()
...
class TestViewPublicStatistics:
    ...
    def test_statistics_no_data_period_shows_empty_state(self, driver, wait):
        driver.get(BASE_URL)
        wait.until(EC.presence_of_element_located((By.ID, "public-statistics-card")))
        
        trend_list = driver.find_element(By.ID, "public-trend-statistics-list")
        items = trend_list.find_elements(By.TAG_NAME, "li")
        Select(driver.find_element(By.ID, "public-stat-granularity")).select_by_value("week")
        old_text = trend_list.text
